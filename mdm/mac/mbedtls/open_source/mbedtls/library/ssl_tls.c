                            ...
                            unsigned char *dstbuf, size_t dlen )
{
    ...
    tmp = ( unsigned char * )mbedtls_calloc( 1, sizeof(  unsigned char ) * 128 );
    if ( tmp == NULL )
    {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }

    h_i = ( unsigned char * )mbedtls_calloc( 1, sizeof(  unsigned char ) * MBEDTLS_MD_MAX_SIZE );
    if ( h_i == NULL )
    {
        mbedtls_free( tmp );
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    ...
    if( sizeof(  unsigned char ) * 128 < md_len + strlen( label ) + rlen ) {
        mbedtls_free( tmp );
        mbedtls_free( h_i );
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    }
    ...
    mbedtls_platform_zeroize( tmp, sizeof(  unsigned char ) * 128 );
    mbedtls_platform_zeroize( h_i, sizeof(  unsigned char ) * MBEDTLS_MD_MAX_SIZE );
    mbedtls_free( tmp );
    mbedtls_free( h_i );
...
int mbedtls_ssl_derive_keys( mbedtls_ssl_context *ssl )
{
    ...
    tmp = (unsigned char *)mbedtls_calloc( 1, sizeof( unsigned char ) * 64 );
    if ( tmp == NULL )
    {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }

    keyblk = (unsigned char *)mbedtls_calloc( 1, sizeof( unsigned char ) * 256 );
    if ( keyblk == NULL )
    {
        mbedtls_free( tmp );
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    ...
    mbedtls_platform_zeroize( tmp, sizeof( unsigned char ) * 64 );
    ...
    mbedtls_platform_zeroize( keyblk, sizeof( unsigned char ) * 256 );
...
void ssl_calc_verify_tls_sha384( mbedtls_ssl_context *ssl, unsigned char hash[48] )
{
    ...
    sha512 = (mbedtls_sha512_context *)mbedtls_calloc(1, sizeof(mbedtls_sha512_context));
    if (sha512 == NULL)
    {
        return;
    }
                ...
                mbedtls_ssl_context *ssl, unsigned char *buf, int from )
{
    ...
    padbuf = ( unsigned char * )mbedtls_calloc( 1, sizeof( unsigned char ) * 48 );
    if (padbuf == NULL)
    {
        mbedtls_free( sha512 );
        return;
    ...
    mbedtls_platform_zeroize(  padbuf, sizeof( unsigned char ) * 48 );
...
int mbedtls_ssl_parse_finished( mbedtls_ssl_context *ssl )
{
    ...
    buf = (unsigned char *)mbedtls_calloc(1, sizeof(unsigned char) * SSL_MAX_HASH_LEN);
    if (buf == NULL)
    {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
